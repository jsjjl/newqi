...
Page({
  ...
  onLoad(options) {
    if (options.taskId) {
      this.setData({
        ...
        taskName: options.taskName || '重大活动/节假日安全排查任务'
      });
    }
  },
  ...
  submit() {
    ...
    if (!that.data.location.trim()) {
      return wx.showToast({ title: '请输入负责的科室或排查区域', icon: 'none' });
    }
    if (!that.data.description.trim()) {
      return wx.showToast({ title: '请详细描述排查情况及结论', icon: 'none' });
    }
  ...
  submitData() {
    let that = this;
    wx.request({
      url: 'https://medicalgas.lygyy.com.cn/prod-api/holidaySafety/report/submit', // 假设的接口
      method: 'POST',
      data: {
