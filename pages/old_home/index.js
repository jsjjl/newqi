...
Page({
  data: {
    // 新增字段
    buildingList: [], // 楼栋列表
    selectedBuilding: '', // 当前选中的楼栋
    showBuildingList: true, // 是否显示楼栋列表（默认显示）
  ...
  getBuildingList() {
    const that = this;
    wx.request({
      url: `https://medicalgas.lygyy.com.cn/prod-api/common/getBuildNo`,
      method: 'get',
      data: {
      ...
      success(res) {
        if (res.data.code === 200) {
          // 处理楼栋名称："1#" → "1号楼"，"净化" → "净化号楼"
          const formattedList = (res.data.data || []).map(building => {
            let buildingName;
            // 仅针对"净化"单独处理
            if (building === '净化') {
              buildingName = '净化号楼';
              ...
              // 其他带#的格式正常替换（如1# → 1号楼）
              buildingName = building.replace(/#/g, '号楼');
            } else {
              // 非"净化"且不带#的保持原样（如果有其他特殊情况）
              buildingName = building;
            }
            return buildingName;
          });
          that.setData({
            buildingList: formattedList
          });
          console.log("处理后的楼栋列表：", that.data.buildingList);
        }
      }
    });
  },
  ...
  // 选择楼栋
  onSelectBuilding(e) {
    const buildingNo = e.currentTarget.dataset.building;
    this.setData({
      selectedBuilding: buildingNo
    });
    // 调用接口获取该楼栋的数据
    this.getlist7(buildingNo);
  },
  ...
  // 修改原getlist7方法，添加楼栋参数
  getlist7(buildingNo) {
    const that = this;
     let buildingName = buildingNo;
      if (buildingName == '净化号楼') {
              buildingName = '净化';
            } else if (buildingName.includes('号楼')) {
              // 其他带#的格式正常替换（如1# → 1号楼）
              buildingName = buildingName.replace(/号楼/g, '#');
            } else {
              // 非"净化"且不带#的保持原样（如果有其他特殊情况）
              buildingName = buildingName;
            }
    wx.request({
      url: that.data.areaId === '1921753035498872833'
        ? 'https://medicalgas.lygyy.com.cn/prod-api/kf/home/alarmbox/show/list'
        : 'https://medicalgas.lygyy.com.cn/prod-api/home/alarmbox/show/list',
      method: 'get',
     
      data: {
        areaId: wx.getStorageSync('areaId'),
        buildingNo: buildingName // 新增楼栋参数
      },
      header: {
        Authorization: wx.getStorageSync('token')
      },
      success(res) {
        if (res.data.code === 200) {
          that.setData({
            qygjx: Object.values(res.data.data) || [], // 存储当前楼栋的数据
            showBuildingList: false // 隐藏楼栋列表，显示数据
          });
        }
      }
    });
