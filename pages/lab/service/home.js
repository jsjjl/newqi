...
Page({
  ...
  select(e) {
    this.setData({
      ...
      list: [], // 清空旧数据
      noMor: false, // 重置加载状态
      noData: false // 重置无数据状态
    }, () => {
      this.getlist() // 在回调中确保状态更新后执行
    })
  },
  ...
  getlist() {
    ...
    const temp = {
      ...
      dirId: that.data.id || undefined // 增加空值处理
    }
    
    // 添加加载状态提示
    wx.showLoading({ title: '加载中...' })
    wx.request({
      url: CONFIG.subDomain + '/klFileInfo/list',
      method: 'get',
      data: temp,
      header: {
        Authorization:wx.getStorageSync('token'),
      ...
      success: function(res) {
        wx.hideLoading()
        if (res.data.code === 200) {
          if(that.data.pageNum == 1 && res.data.rows.length == 0){
            that.setData({
              noData:true
            })
          }
          // 增加数据有效性校验
          const validData = res.data.rows.filter(newItem => 
            !that.data.list.some(existingItem => 
              existingItem.id === newItem.id
            )
          )
          
          that.setData({
            list: that.data.pageNum === 1 ? 
              validData : 
              [...that.data.list, ...validData],
            // ...其他状态设置...
          })
        }
      },
