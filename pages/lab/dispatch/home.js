...
Page({
  ...
  save() {
    ...
    console.log(that.data.location, that.data.description, that.data.img_arr);
    ...
    if (that.data.isAnonymous == false) {
      if (that.data.empno == '' || that.data.name == '' || that.data.phone == '') {
        wx.showToast({
          title: '请填写完整上报内容',
          icon: 'none',
          duration: 2000
  ...
  submit() {
    ...
    wx.request({
      url: 'https://medicalgas.lygyy.com.cn/prod-api/wx/event/report',
      method: 'POST',
      data: {
