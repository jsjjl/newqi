...
Page({
  ...
  onTypeChange(e) {
    ...
    if (type === 2) {
      if (this.data.deviceIndex !== null) {
        this.setData({ partList: [], partIndex: null });
        const deviceId = this.data.deviceList[this.data.deviceIndex].id; // 修正这里
        this.fetchPartList(deviceId);
      }
      if (this.data.smsbid) {
        this.setData({ partList: [], partIndex: null });
        this.fetchPartList(this.data.smsbid);
      }
    }
    // ...existing code...
  },
  ...
  onSubmit() {
    ...
    const { deviceList, deviceIndex, maintenanceTypeList, maintenanceTypeIndex, userList, userIndex, partList, partIndex, type, maintainDate, contact, remark, userInput, selectedUserId, operatorName } = this.data;
    if (maintenanceTypeIndex === null || !maintainDate || !contact || !operatorName) {
    ...
    wx.request({
      ...
      success: res => {
        ...
        if (res.data.code === 200) {
          ...
          //关闭当前页面跳转到新页面
          // url: '/pages/maintain/list?type=2'
          this.getby();
          // setTimeout(() => wx.redirectTo({
          //   url: "/pages/sebei/detail_baoyang?id=" + res.data.data,
          // }), 1500);
        } else {
