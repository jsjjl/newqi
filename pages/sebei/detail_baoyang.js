...
Page({
  ...
  wcmx(){
    ...
    wx.showModal({
      ...
      success: function (res) {
        if (res.confirm) {
           wx.request({
      url: `https://medicalgas.lygyy.com.cn/prod-api/device/maintenance/complete/`+id,
      method: 'post',
    ...
    wx.request({
      ...
      success: res => {
        if (res.data.code === 200) {
          ...
          wx.request({
            url: 'https://medicalgas.lygyy.com.cn/prod-api/device/maintenance/info?id=' + e,
            method: 'GET',
            header: { Authorization: wx.getStorageSync('token') },
            success: res => {
              if (res.data.code === 200) {
                ...
                let temps = this.data.maintenanceTypeList;
                console.log(temps);

                for (let j = 0; j < temps.length; j++) {
                  if (temp.maintenanceType == temps[j].dictValue) {
                    console.log(temps[j]);
                    temp.maintenanceTypeName = temps[j].dictLabel
                  }
                }

               
                  wx.request({
                    url: `https://medicalgas.lygyy.com.cn/prod-api/device/info/${temp.deviceId}/parts/list`,
                    method: 'GET',
                    header: { Authorization: wx.getStorageSync('token') },
                    success: res => {
                      if (res.data.code === 200) {
                        this.setData({ partList: res.data.data });
                        let tempps = res.data.data;
                        console.log(tempps);
        
                        for (let j = 0; j < tempps.length; j++) {
                          if (temp.partId == tempps[j].id) {
                            console.log(temps[j]);
                            temp.partName = tempps[j].partName
                          }
                        }
      ...
      success: function success(res) {
        ...
        if (res.data.code == 200) {
             ...
             that.setData({
              ...
              that.updateFilteredItems(); // 初始化时也更新一次
            });
  ...
  switchTab(e) {
   ...
   updateFilteredItems() {
    ...
    temp.forEach(item => {
      if(item.picturesUrl){
item.picturesUrl = item.picturesUrl.split(",");
      }
    ...
    // if (currentTab == 0) {
    //   // 开始维修
    //   filteredItems = list.filter(item => item.status == 1);
    // } else if (currentTab == 1) {
    //   // 维修中
    //   filteredItems = list.filter(item => item.status == 2);
    // } else if (currentTab == 2) {
    //   // 维修完成
    //   filteredItems = list.filter(item => item.status == 3);
    // }
  ...
  previewImg(e) {
    let index = e.currentTarget.dataset.index;
       let dq = e.currentTarget.dataset.dq;
