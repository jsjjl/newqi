...
Page({
    ...
    wx.request({
      ...
      success: res => {
        if (res.data.code === 200) {
          ...
          wx.request({
            url: 'https://medicalgas.lygyy.com.cn/prod-api/device/repair/info?id=' + e,
            method: 'GET',
            header: { Authorization: wx.getStorageSync('token') },
            success: res => {
              if (res.data.code === 200) {
                ...
                let temps = this.data.repairTypeList;
                console.log(temps);
                ...
                for (let j = 0; j < temps.length; j++) {
                  if (temp.repairType == temps[j].dictValue) {
                    console.log(temps[j]);
                    temp.repairTypeName = temps[j].dictLabel
                  }
                }
  ...
  wcmx() {
    ...
    wx.showModal({
      ...
      success: function (res) {
        if (res.confirm) {
          wx.request({
            url: `https://medicalgas.lygyy.com.cn/prod-api/device/repair/complete/` + id,
            method: 'post',
            header: { Authorization: wx.getStorageSync('token') },
            success: res => {
              if (res.data.code === 200) {
  ...
  getList(e) {
    let that = this
    wx.request({
      url: 'https://medicalgas.lygyy.com.cn/prod-api/device/repair/' + e + '/details/list',
      method: 'get',
      // data: {
  ...
  addmx() {
    wx.navigateTo({
      url: '/pages/sebei/add_weixiu?id=' + this.data.detail.id,
    })
  },
  nav1() {
    wx.navigateTo({
