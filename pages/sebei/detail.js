...
Page({
  ...
  onLoad: function (e) {
    ...
    if (deviceId) {
      ...
      if (!wx.getStorageSync('token')) {
        wx.setStorageSync('redirectAfterLogin', `/pages/sebei/detail?id=${deviceId}`);
        wx.redirectTo({
  ...
  toform(e) {
    let that = this
    console.log("ccc", e.currentTarget.dataset.item);
    wx.navigateTo({
      url: '/pages/lab/task_post/task_post?taskId=' + e.currentTarget.dataset.item.taskId + '&pointId=' + e.currentTarget.dataset.item.pointId,
    })
  },

  handleDateUpdate(updateFields) {
    let that = this;
    wx.chooseImage({
      count: 1,
      sizeType: ['compressed'],
          ...
          success: function(uploadRes) {
            if (uploadRes.statusCode == 200) {
              ...
              if (data.code === 200) {
                ...
                // Set the fields based on which button was clicked
                for (let key in updateFields) {
                  payload[key] = updateFields[key] === 'today' ? today : imgUrl;
                }
                ...
                wx.request({
                  ...
                  success: function(resUpdate) {
                    ...
                    if (resUpdate.data.code == 200) {
                      ...
                      // update local detail
                      let newDetail = { ...that.data.detail };
                      for (let key in payload) {
                        if (key !== 'id') {
                          newDetail[key] = payload[key];
                        }
                      }
                      that.setData({ detail: newDetail });
                    } else {
                      wx.showToast({ title: resUpdate.data.msg || '更新失败', icon: 'none' });
                    }
                  },
                  fail: function() {
                    wx.hideLoading();
  ...
  updateMaintenanceDate() {
    this.handleDateUpdate({
      ...
      maintenancePhoto: 'imgUrl'
    });
  },

  updateReplacementDate() {
    this.handleDateUpdate({
      replacementDate: 'today',
      activationDate: 'today',
      replacementPhoto: 'imgUrl'
    });
  },
  ...
  xjgetlist(e) {
    ...
    wx.request({
      ...
      url: CONFIG.subDomain + '/wx/device/pointTaskList',

      method: 'get',
      data: {
        deviceId: this.data.detail.id ? this.data.detail.id : wx.getStorageSync('detail').id
        // pointId: this.data.detail.id?this.data.detail.id:wx.getStorageSync('detail').id
      },
      header: {
        Authorization: wx.getStorageSync('token'),
        clientid: wx.getStorageSync('clientid')
  ...
  _tabs(e) {
    ...
    if (temp == 2) {
      ...
      this.nfetchMaintenanceTypeList()
    }
    ...
    if (temp == 3) {
      ...
      this.nfetchRepairTypeList()
    }
  },

  nfetchMaintenanceTypeList() {
    wx.request({
      ...
      success: res => {
        if (res.data.code === 200) {
          this.setData({ maintenanceTypeList: res.data.data }); this.xgetlist();
        }
      }
    });
  },
          ...
          if (res.data.rows.length == 0) { //请求的数据为空，没有数据
            ...
            for (let i = 0; i < tempData.length; i++) {
              for (let j = 0; j < temp.length; j++) {
                if (tempData[i].maintenanceType == temp[j].dictValue) {
                  tempData[i].maintenanceTypeName = temp[j].dictLabel
                }
              }
            }
            ...
            // 请求的结果有数据额
            if (tempData.length == that.data.pageSize) { // 请求的数据为10条，说明下一页可能还有数据，列表添加上新的数据，把其他状态设为不显示
              that.setData({
                list: [...that.data.list, ...tempData],
                noMor: false,
                noData: false
              })
  ...
  ngetlist() {
    ...
    // }
    wx.request({
      url: CONFIG.subDomain + '/device/repair/page',
      method: 'get',
      data: temp,
      header: {
        Authorization: wx.getStorageSync('token'),
      ...
      success: function success(res) {
        ...
        if (res.data.code == 200) {
          if (res.data.rows.length == 0) { //请求的数据为空，没有数据
            ...
            for (let i = 0; i < tempData.length; i++) {
              for (let j = 0; j < temp.length; j++) {
                if (tempData[i].repairType == temp[j].dictValue) {
                  tempData[i].repairTypeName = temp[j].dictLabel
                }
              }
            }
                ...
                wx.request({
                  ...
                  success: res => {
                    if (res.data.code === 200) {
                      ...
                      wx.request({
                        url: 'https://medicalgas.lygyy.com.cn/prod-api/wx/device/info?deviceId=' + e,
                        method: 'GET',
                        header: { Authorization: wx.getStorageSync('token') },
                        success: res => {
                          if (res.data.code === 200) {
                            ...
                            if (this.data.detail.parts) {
                              // value如果有 00:00:00过滤掉

                              for (let key in this.data.detail.parts) {
                                if (this.data.detail.parts[key] === '00:00:00') {
                                  this.data.detail.parts[key] = '-';
                                }
                              }
                              this.setData({
                                partKeys: Object.keys(this.data.detail.parts)
                              });
                            }
                          }
                        }
  ...
  goby() {
    wx.navigateTo({
      url: '/pages/sebei/baoyang?id=' + this.data.detail.id,
    })
  },

  gowx() {
    wx.navigateTo({
      url: '/pages/sebei/weixiu?id=' + this.data.detail.id,
    })
  },
  toMore() {
    wx.navigateTo({
      url: '/pages/lab/task_search2/task_search?deviceId=' + this.data.detail.id,
    })
  },
  ...
  previewImg(e) {
    ...
    let img_arr = e.currentTarget.dataset.pictures;
    wx.previewImage({
      current: img_arr,
      urls: [img_arr]
    })
